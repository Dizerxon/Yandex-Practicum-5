...
func (t Training) distance() float64 {
	...
	res := float64(t.Action) * t.LenStep / MInKm
	return res
}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	speed := r.meanSpeed()
	res := ((CaloriesMeanSpeedMultiplier * speed  + CaloriesMeanSpeedShift ) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
	return res
}
...
func (w Walking) Calories() float64 {
	...
	mHeight :=  w.Height / CmInM
	if mHeight == 0{
		return 0
	}
	res := ((CaloriesWeightMultiplier * w.Weight + (math.Pow(speed, 2)/mHeight)*CaloriesSpeedHeightMultiplier * w.Weight) * w.Duration.Hours() * MinInHours)
	return res 
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	res :=	float64(s.LengthPool) * float64(s.CountPool) /  float64(MInKm) /  float64(s.Duration)
	return res
}
